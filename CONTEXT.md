...
## Save File
A JSON file representing the full Map state: the Level Stack (all Z Level grids), Stamp positions/types/rotations/Z levels and optional per-Stamp color overrides, Step Runs, Ramp Runs, Canvas dimensions, Wall color/opacity settings, and the Player View preference. The Level Stack is serialized as an object with string-keyed Z Level integers, each value being a flat number array. Old save files with a flat `grid` key (single-layer format) are loaded as Z=0; saves without a `steps` key load with no Step Runs, saves without a `ramps` key load with no Ramp Runs, and saves without `playerView` load with room numbers visible.

## Undo / Redo
History is a stack of full Map snapshots (Level Stack + Stamps + Step Runs + Ramp Runs). One snapshot is taken per completed gesture: mouseup for Brush strokes, and each Stamp, Step Run, or Ramp Run action (place, move, rotate, scale, mirror, recolor, delete) individually. Snapshots capture the full Level Stack across all Z Levels.
