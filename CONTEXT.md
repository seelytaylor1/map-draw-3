...
## Room Encounter
A seeded room-content roll produces an empty room 50% of the time, monsters 30%, traps 10%, and hazards 10%. The dungeon's five-entry ambient monster encounter table is drawn from the party-level band, preferring monsters that form one standard encounter. Ambient monster rooms use each affordable table entry once before any entry may repeat, preventing a sole low-level result from monopolizing the dungeon. A higher-level mission danger contract may replace one table entry with its required featured monster. Room hazards receive a unique hazard record within the generated map and use the Danger stamp; room traps use the Trap stamp with its T. Mission contracts emit room Danger Entries rather than the space layer branching on challenge names; an `all` entry resolves from the central danger-kind pool, so hazards count as danger anywhere a Loop Challenge concentrates dangerous encounters. Contract-cleared rooms currently resolve to Empty and are positioned for future Empty variants such as NPC or lore. Independently, each room has a 2-in-6 chance of a Chest, which may coexist with any encounter. Monster encounters remain in the room metadata and ledger but do not place a generated room stamp. A Loop Challenge reward also places a Chest in the Goal room.
