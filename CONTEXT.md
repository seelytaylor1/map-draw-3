...
## Stamp
A placeable symbol snapped to the Tile Grid. Each Stamp type has a fixed size in tiles. Stamps live on a separate layer from the Tile Grid and can be moved, rotated, scaled, mirrored, recolored, and deleted individually after placement. A Stamp color override tints only that placed Stamp; without an override, the original asset colors are used. Each Stamp belongs to a Z Level and is placed at the Active Z Level when dropped. Two categories: Floor Stamp and Object Stamp.
...
## Save File
A JSON file representing the full Map state: the Level Stack (all Z Level grids), Stamp positions/types/rotations/Z levels and optional per-Stamp color overrides, Step Runs, Ramp Runs, Canvas dimensions, Wall color/opacity settings, the Player View preference, and editable Room Ledger entries and general notes. The Level Stack is serialized as an object with string-keyed Z Level integers, each value being a flat number array. Old save files with a flat `grid` key (single-layer format) are loaded as Z=0; saves without a `steps` key load with no Step Runs, saves without a `ramps` key load with no Ramp Runs, saves without `playerView` load with room numbers visible, and saves without ledger fields load with an empty editable ledger.

## Undo / Redo
History is a stack of full Map snapshots (Level Stack + Stamps + Step Runs + Ramp Runs + Room Ledger entries and notes). One snapshot is taken per completed gesture: mouseup for Brush strokes, and each Stamp, Step Run, or Ramp Run action (place, move, rotate, scale, mirror, recolor, delete) individually. Snapshots capture the full Level Stack across all Z Levels.
...
## Room Encounter
A seeded room-content roll produces an empty room 50% of the time, monsters 30%, traps 10%, and hazards 10%. The dungeon's five-entry ambient monster encounter table draws from LV0 through the party-tier ceiling (LV0–3, LV0–6, LV0–9, or LV0–10), preferring monsters that form one standard encounter. Each monster table row starts with a quantity roll from d2, d3, d4, d6, d8, d10, d12, 2d3, 2d4, 2d8, or 2d10; the selected roll average is close to encounter budget divided by monster level. This is reference text and does not change seeded room generation. Ambient monster rooms use each affordable table entry once before any entry may repeat, preventing a sole low-level result from monopolizing the dungeon. A higher-level mission danger contract may replace one table entry with its required featured monster. Each monster group also gets a seeded activity category and one expression from that category, plus a seeded reaction category and one expression. The category and expression are shown in room details, leaving targets and motivations for the GM to supply. These context rolls use a room-specific random stream so they do not change other generated content. Room hazards receive a unique hazard record within the generated map and use the Danger stamp; room traps use the Trap stamp with its T. Mission contracts emit room Danger Entries rather than the space layer branching on challenge names; an `all` entry resolves from the central danger-kind pool, so hazards count as danger anywhere a Loop Challenge concentrates dangerous encounters. Contract-cleared rooms currently resolve to Empty and are positioned for future Empty variants such as NPC or lore. Treasure is generated as level-scaled finds with at most one find per room. The first Fabulous or Legend find is placed in the Goal room; additional high-tier finds use other rooms. Finds beyond room capacity are omitted. The level GP total is a shared budget for itemized GP finds and rolled magic-item values. Outcomes that exceed the remaining budget are rerolled for that room; if no affordable outcome is found, the room receives no find. Eligible high-tier finds may use an enabled magic-item source, with Shadowdark Core enabled by default. Consumable items are Weak and cost 1d6 × 100 gp; all other Core items are Strong and cost 2d6 × 100 gp. Their rolled values count in the generated treasure value. Monster encounters remain in room metadata and the ledger but do not place a generated room stamp.

## Dungeon Legend
A collapsed lower-right map control that explains generated map symbols using their actual stamp images. It includes the ascending start-room entrance, generated door variants, key, locked door, trap, treasure, hazard, hub, and flooded-hallway markers.

## Itemized Treasure

GP-valued treasure finds are rendered as concrete coins, gems, gear, luxury goods, weapons, scrolls, potions, wands, or other magic items using the party's level band. The item's table value can differ from its rolled GP target by up to 3 gp; only the item's value counts against the shared treasure budget. Item qualities add condition or craftsmanship flavor, and coin finds include a location and mint. Scrolls and potions can receive the supplied mutation effects. Magic weapon, magic armor, and genie-lamp entries are excluded from treasure generation.
