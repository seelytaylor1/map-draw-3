...
## Save File
A JSON file representing the full Map state: the Level Stack (all Z Level grids), Stamp positions/types/rotations/Z levels and optional per-Stamp color overrides, Step Runs, Ramp Runs, Canvas dimensions, and Wall color/opacity settings. The Level Stack is serialized as an object with string-keyed Z Level integers, each value being a flat number array. Old save files with a flat `grid` key (single-layer format) are loaded as Z=0; saves without a `steps` key load with no Step Runs, and saves without a `ramps` key load with no Ramp Runs.
...
## Room Encounter
A seeded room-content roll produces an empty room 50% of the time, monsters 30%, traps 10%, and hazards 10%. Room hazards receive a unique hazard record within the generated map and use the Danger stamp; room traps use the Trap stamp with its T. Mission contracts emit room Danger Entries rather than the space layer branching on challenge names; an `all` entry resolves from the central danger-kind pool, so hazards count as danger anywhere a Loop Challenge concentrates dangerous encounters. Contract-cleared rooms currently resolve to Empty and are positioned for future Empty variants such as NPC or lore. Independently, each room has a 2-in-6 chance of a Chest, which may coexist with any encounter. Monsters use the Triangle Arrowhead stamp. A Loop Challenge reward also places a Chest in the Goal room.
