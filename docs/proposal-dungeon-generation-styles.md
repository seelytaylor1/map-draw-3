...
`npm run diagnose:mission-topology -- [count] [firstSeed] [challenge|all] [loopCount] [compact|standard|dense] [style|all]` evaluates realized room-and-corridor plans across the selected styles instead of trusting the requested Mission graph. It reports each cycle's route geometry, usable-route count, and the anchor's position from Start to objective, then independently rechecks each Loop Challenge contract. It also checks the rendered snapshot: Key markers, locked-door markers, and the secret, dangerous, blocked, and one-way connection semantics must match their Mission edges. Use `all` for the challenge or style argument to sweep the full respective set. A cycle anchor after the halfway point is a `late-cycle-choice` failure: it is a linear approach with a fork at the end, not a meaningful map-shaping choice. The command exits non-zero when any generated map fails this check, so it can serve as a regression gate while layout productions evolve.
