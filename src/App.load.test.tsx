...
describe('App load lifecycle', () => {
  ...
  it('keeps the editable room ledger open beside the dungeon legend', async () => {
    render(<App />)
  ...
  it('exposes the shared mission-first request and live preflight controls', () => {
    ...
    const coreMagicItems = screen.getByRole('button', { name: 'Shadowdark Core magic items' })
    expect(coreMagicItems).toHaveAttribute('aria-pressed', 'true')
    fireEvent.click(coreMagicItems)
    expect(coreMagicItems).toHaveAttribute('aria-pressed', 'false')
    expect(coreMagicItems).toHaveTextContent('Off')
