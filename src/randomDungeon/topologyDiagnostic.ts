...
export function assessMapTopology(mission: Mission, plan: SpacePlan): MapTopologyAssessment {
  ...
  const cycles = mission.cycles.map(cycle => {
    ...
    const dramaticRoomContract = cycle.challenge === 'dramatic-arc' && anchor === objective && cycle.roles.objectiveNode === mission.goalNodeId
    if (cycle.challenge === 'dramatic-arc' && ((!dramaticRoomContract && !routeA.some(connection => connection.traversable === 'blocked')) || !routeB.some(connection => connection.traversable !== 'blocked'))) addMissingRealization(`${cycle.id} must block Route A while keeping Route B open.`)
